...
class PopRuntime{
    ...
    constructor(context = null,safeFlag = true,commentRetainFlag = true,maxDeep = 1000100)
    {
       // console.log('safeFlag:'+safeFlag)
        ...
        this.OPCODES = {}
        this.rpc_api_util = typeof RPCApiUtil!='undefined' ?  new RPCApiUtil(null) :null
        this.initFuncs()
        if(typeof window =='undefined') globalThis.window = globalThis

        //2023-10-7新增
        if(window.g_dchatManager && typeof window.g_dchatManager.initAppRuntime == 'function')
        {
            let initFlag = g_dchatManager.initAppRuntime(this)
            console.log('PopRuntime.constructor--initFlag:',initFlag)
        }
    ...
    */
    async jsrun(context,opcode,...etc)//token_x,opval,extra_data,
    {
        ...
        if( (typeof popFunc) != 'function') return context['$ret'] = popFunc; //!popFunc||(typeof popFunc) != 'function'
        ...
        if(etc.length>=1)//>=2
        {
           ...
           // let opret = etc.pop();
            ...
            // ret = context[opret]
            ret = await parent[opList[opList.length-1]](...params)
        }else{
            ret = await parent[opList[opList.length-1]]()//popFunc()//etc.length>0 ? popFunc( context[etc[0]]):
        }
        context['$ret'] = ret
        return ret
    }
    ...
    initFuncs(){
        ...
        this.binderAddOpcode('set',function(context,opcode,token_x,token_y,opval,extra_data,...etc){
            ...
            context[token_x] =  opvalObj == 'false' ? false:opvalObj
            return context[token_x];
        })
        ...
        //新增一个将context设置为当前引用的变量。2023-1-31（主要用于context函数调用中）
        this.binderAddOpcode('this.set',function(context,opcode,token_x,token_y,opval,extra_data,...etc){
            context[token_x] = context
            return true
        })
        //deep-copy 深度拷贝
        ...
        //数组操作
        this.binderAddOpcode('array.set',function(context,opcode,token_x,token_y,opval,extra_data,...etc){
            let index = context [token_y]
            return context[token_x][index] = context[opval];
        })
        this.binderAddOpcode('array.get',function(context,opcode,token_x,token_y,opval,extra_data,...etc){
            let index = context [token_y]
            return context[opval] = context[token_x][index];
        })
        ...
        this.binderAddOpcode('pop.while',async function(context,opcode,token_x,token_y,...etc){
            while(context[token_x] && !This.quit_flag) await This.op(context,token_y,...etc)
            return {ret:true,msg:'pop.while run end'}
        })
        this.binderAddOpcode('pop.sleep',async function(context,opcode,token_x,token_y,...etc){
            //2024-5-15支持time取值context中的值
            let time = context[token_x] ? context[token_x]: token_x
            return await new Promise((resolve)=>setTimeout(resolve,time))//str_filter.sleep(token_x)
        })
        this.binderAddOpcode('pop.do.until',async function(context,opcode,token_x,token_y,...etc){
            ...
            while(!flag  && !This.quit_flag){await This.op(context,token_y,...etc);flag = (''+token_x).indexOf('@')==0?This.$$[token_x.split('@')[1]]: context[token_x]} 
        ...
        this.binderAddOpcode('pop.do.while',async function(context,opcode,token_x,token_y,...etc){
            ...
            while(context[token_x]  && !This.quit_flag)
        ...
        this.binderAddOpcode('func.while',function(context,opcode,token_x,token_y,opval,extra_data,...etc){
            ...
            context[token_x] = async function(context)//由context进行传入参数//params,extra_data)
            {
                ...
                while(context[token_y] && maxSepCnt>0  && !This.quit_flag){
                    maxSepCnt--
        ...
        this.binderAddOpcode('new',function(context,opcode,token_x,...etc){
            let ret = null;
            switch(token_x)//(token_x).toLowerCase()) //2024-7-10修改，支持使用Map或Array等进行变量创建
            {
                case 'obj':
                ...
                default:
                    //直接读取值（例如Map，或者etc） 2024-7-10新增
                    const xclass = context[token_x] ? context[token_x] :token_x
                    if(xclass) ret =  new This.$[xclass](...etc) //在window中读取（支持参数）
                    else ret = {};
            }
            return context['$ret'] = ret
        ...
        this.binderAddOpcode('local.file.upload',async function(context,opcode,token_x,token_y,opval,extra_data,...etc){
            ...
            const file_util = null;// require('../libs/file_util')
    ...
    async op(context,opcode,...etc){
        //2024-5-15新增
        if(this.quit_flag){
            console.log('op-quit_flag-is-true, end-the-poplang')
            return '[quicked]'
        }
        if(!context) context = this.context;
        if(!opcode) return false
        //转义opcode，在conext上下文中读取 2024-6-27
        ...
        try{
            ...
            if((opcode).startsWith('/') || (opcode).startsWith('dtns://') )//|| (opcode).startsWith('http://') || (opcode).startsWith('https://')) {//这个是查询函数的绑定（也是直接解析的后端查询）
            {    //这里应该增加TNS域名支持、keys支持
                let url =((opcode).startsWith('/')  ?'dtns://web3:'+rpc_client.roomid+opcode : opcode)//利用上这个解析库进行查询。
            ...
            //         params[key] =value
            //     }
            //     // console.log('poplang-url:'+JSON.stringify(url.searchParams)+' url:'+url+' params:',params,'s-params:',url.searchParams)
            //     //这里取特定的token
            // // params.token =  params.token? params.token :token_x 
            // //一般而言，token_x指向的变量保存参数，token_y保存的是appid_key等信息，同时保存了urlbase等。
                let ps = {} //Object.assign({},params)// Object.assign({extra_data:context[extra_data]},context[token_x],context[token_y],params)
                //添加更多参数进来
                ...
                // //将结果放入到opval指向的变量中。
                // console.log('poplang-ps:'+JSON.stringify(ps))
                // let data = localStorage.getItem('userInfo') ? JSON.parse(localStorage.getItem('userInfo')):{}
                // ps.user_id = data.user_id
                // ps.s_id = data.s_id
                context[opret]  = await g_dtnsManager.run(url,ps)// await this.rpc_api_util.rpc_query(path,ps);//这里配置了本机及远程的访问(暂时缺少对文件的支持)
            ...
            {
                return await this.jsrun(context,opcode,...etc)
            }else{
    ...
    async opbatch(context,list,iswait=true)
    {
        ...
        try{
            ...
            for(let i=0;i<list.length;i++)
            {
                //2024-5-15新增
                if(this.quit_flag){
                    console.log('opbatch-quit_flag-is-true, end-the-poplang')
                    return '[quicked]'
                }
                if(!list[i] || !list[i][0]) continue
                if(list[i][0].startsWith('//')) continue
                //编译指令，不收集结果（特别用于递归函数时---得到的结果嵌套非常深，会导致JSON.stringify崩溃）2022-9-16
                if( list[i][0]=='###' && list[i].length==1) $log_result_flag = !$log_result_flag,ret={ret:true,msg:'# !flag'}
                //进入函数定义区。
                ...
                else if($pop_func_area && list[i][0]=='pop.func.end')
                {
                    ...
                    if(iswait && list[i][0].indexOf('&')!=0 || iswait && list[i][0]=='&&') ret = await this.op(context,...list[i])
                    else{
                ...
                if($log_result_flag) results.push(typeof deepClone!='undefined' ?  deepClone(ret) : ret)//(deepcopy(ret))// this.deepCopy(ret) )//进行了结果对象的复制--否则会导致stringify拿到的一般情况下是context的重复
    ...
    parseLine(line)
    {
        ...
        for(let i=0;i<line.length;i++)
        {
            if((''+line[i]).trim().length == line[i].length ) tmp+=line[i]
                ...
                if(tmp.trim().length == tmp.length && tmp.length>0){words.push(tmp)}
                tmp = ''
            }
            //生成字符串（使用前缀**代表）  起始标识  2024-5-15新增
            if(tmp=='**'){
                tmp = line[line.length-1] == '\r' ? line.substring(i+1,line.length-1): line.substring(i+1,line.length)
                if(tmp.length>0)
                {
                    words.push(tmp)
                    tmp = ''
...
// test2();
//2024-7-3新增了对$$.replace 和$$.length 的测试
async function testnow()
{
    let pop = new PopRuntime(null)
    let rets = await pop.runScript(null,"# Initialize the array of phone numbers\nset numbers **[19320153333,13789051335,15689901356,13490377131,13390001233]\nset target 18216815719\nset length 5\nset i 0\nset found false\nset flag true\nset one 1\n\n# Define function to check if current element matches target\npop.func.define check_loop\narray.get numbers i current\n== current target temp\npop.ifelse temp handle_found handle_not_found\npop.func.end\n\n# Handle found case\npop.func.define handle_found\nset found true\nset flag false\npop.func.end\n\n# Handle not found case\npop.func.define handle_not_found\n+ i one i\n< i length flag\npop.func.end\n\n# Execute the loop\npop.do.while flag check_loop\n\n# Prepare result strings\nset result_ok **Found\nset result_notok **Not found\n\n# Determine final result\n?= found result_ok result_notok result\nthis.get")
    // let rets = await pop.runScript(null,"#\n#\n# 初始化数组和变量\nset Inputs **[5,3,8,1,2]\nset n 5\nset swapped true\nset temp 0\nset i 0\nset limit 0\nset current 0\nset next_i 0\nset next_val 0\nset compare_result false\nset loop_flag false\n\n# 定义冒泡排序外层循环函数\npop.func.define bubble_pass\nset swapped false\nset i 0\n- n 1 limit\n< i limit loop_flag\npop.do.while loop_flag inner_loop\npop.func.end\n\n# 定义内层循环函数\npop.func.define inner_loop\narray.get Inputs i current\n+ i 1 next_i\narray.get Inputs next_i next_val\n> current next_val compare_result\npop.ifelse compare_result swap no_swap\n+ i 1 i\n< i limit loop_flag\npop.func.end\n\n# 定义元素交换函数\npop.func.define swap\n= temp current\narray.set Inputs i next_val\narray.set Inputs next_i temp\nset swapped true\npop.func.end\n\n# 定义无交换操作函数\npop.func.define no_swap\npop.func.end\n\n# 执行冒泡排序\npop.do.while swapped bubble_pass\n= x Inputs")
    console.log('rets:',rets)

    // let ret = pop.parseLine('set inputs **[1,2,3]')
    // console.log('ret:',ret)
}
