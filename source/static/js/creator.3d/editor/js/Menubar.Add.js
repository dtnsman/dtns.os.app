...
function MenubarAdd( editor ) {
	...
	option.setTextContent( '3D插件（库）');
	option.onClick( function () {

		if(typeof window.g_show_3d_plugins_list_func =='function')
		{
			window.g_show_3d_plugins_list_func()
		}
	...
	window.g_add_3d_plugin =async function(item)
	{
		let filename = item.url
		let jdata = null
		await g_dchatManager.goFile(filename,true)//从网络或者从内存中加载
		...
		try{
			...
			console.log('g_add_3d_plugin-file_url:',file_url)
			...
			console.log('g_add_3d_plugin-jsonStr:',jsonStr)
			jdata= JSON.parse(jsonStr)
		}catch(ex){
			console.log('g_add_3d_plugin-ex:'+ex,ex)
			return false
		}
		...
		loader.parse( jdata, function ( result ) {

			if ( result.isScene ) {

				//editor.execute( new SetSceneCommand( editor, result ) );
				console.log('json is scene, data:',jdata)

			} else {

				editor.execute( new AddObjectCommand( editor, result ) );
				//output.scripts = editor.scripts[object.uuid]
				console.log('g_add_3d_plugin--import object-json and init scripts add:',jsonData)
				if(jsonData.scripts)
				{
					for(let i=0;i<jsonData.scripts.length;i++)
					{
						editor.execute( new AddScriptCommand( editor, result, jsonData.scripts[i] ) );
					}
				}
