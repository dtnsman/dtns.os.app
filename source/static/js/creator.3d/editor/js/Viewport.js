...
function Viewport( editor ) {
	...
	function handleClick() {
		...
		if ( onDownPosition.distanceTo( onUpPosition ) === 0 ) {
			...
			{
				console.log('intersects-script:',editor.scripts[intersects[0].object.uuid],intersects)
			}
	...
	signals.rendererCreated.add( function ( newRenderer ) {
		...
		window.g_editor_render = newRenderer;
	...
	function render(need_grid=true) {
		if(!editor.scene){ 
			console.log('Editor.viewport.render is stopped! scene is null now!')
			return false
		} 
		...
		if ( camera === editor.viewportCamera ) {
			...
			if ( showSceneHelpers === true && need_grid) renderer.render( sceneHelpers, camera );
			if ( vr.currentSession === null && need_grid) viewHelper.render( renderer );
	...
	window.g_editor_render_func = render
	window.g_editor_stop_func = function()
	{
		console.log('call g_editor_stop_func now:')
		renderer.setAnimationLoop( null );
		renderer.dispose();
		pmremGenerator.dispose();
		container.dom.removeChild( renderer.domElement );

		editor.camera = undefined;
		editor.camera = undefined;
	}

	this.startRecord = window.g_3d_editor_start_record = function()
	{
		if(typeof g_editor_render == 'undefined' ) return 
		console.log('g_editor_render:',g_editor_render,g_editor_render.domElement)
		let canvas =g_editor_render.domElement// g_3d_domElement.getElementsByTagName('canvas')[0]
		...
		if(!canvas) return 
		if(typeof g_3d_editor_recorder!='undefined') g_3d_editor_recorder.stop(0)
		let allChunks=[];
		let stream=canvas.captureStream(24); // 60 FPS recording
		let recorder=new MediaRecorder(stream, {
			mimeType: 'video/webm;codecs=vp9',
		});
		window.g_3d_editor_recorder = recorder
		window.g_3d_editor_recorder_chunks = allChunks
		// canvas 录制回调
	...
	this.stopRecord = window.g_3d_editor_stop_record = function()
	{
		if(typeof g_3d_editor_recorder=='undefined' || !window.g_3d_editor_recorder){
			console.log('recorder is null')
			return 
		}
		g_3d_editor_recorder.stop(0)
		const videoBlob = new Blob(g_3d_editor_recorder_chunks, { 'type' : 'video/webm' })
		var myUrl = URL.createObjectURL(videoBlob)
		rpc_client.downloadFile(myUrl,'3deditor-record-'+Date.now()+'.webm')
		window.g_3d_editor_recorder_chunks = null
		return videoBlob
	}
