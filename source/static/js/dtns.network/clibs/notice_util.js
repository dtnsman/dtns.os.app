// var str_filter = require("../common/util/str_filter");
...
// 设置请求回调处理, 这里只是演示，用户需要自定义相应处理回
//http://sms.opencom.cn/alarm/alarm_sms.jsp?key=OCAlarmCode@266&ip=119.29.162.204&port=3000&process=/data/jetty
/**
 * 发送短信
 * @param nation_code
 * @param phone
 ...
 */
window.notice_util = {}
window.notice_util.send_sms = async function (nation_code,phone,code,is_template) {
    let param = {phone,code,sms_sign:"超级群聊",order_number:"your-order-no","version":1};
    param.template = is_template ? code:null;
    let sendRet = await rpc_query(MSG_API_BASE+'/op',{token_x:MSG_TOKEN_ROOT,token_y:MSG_TOKEN_ROOT,opcode:"sms",
 ...
 * @returns {Promise<boolean>}
 */
window.notice_util.send_email = async function (email,subject,content) {
    let param = {mail_list:email,title:subject,content,"order_number":"your-order-no","version":1};
...
// console.log(JSON.stringify(check_nation_code(86)))
// console.log(JSON.stringify(check_nation_code(76)))
// console.log(JSON.stringify(check_nation_code(7)))
// console.log(JSON.stringify(check_nation_code(137)))
// console.log(JSON.stringify(check_nation_code(254)))
// console.log(JSON.stringify(check_nation_code(1000)))
// console.log(JSON.stringify(check_nation_code(1)))
// console.log(JSON.stringify(check_nation_code(2)))
// console.log(JSON.stringify(check_nation_code(327)))
