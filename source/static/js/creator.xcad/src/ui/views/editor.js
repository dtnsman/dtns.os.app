...
const editorWrapper = (state, editorCallbackToStream) => {
  ...
  if (state.activeTool === 'editor') {
    ...
    const xrunfunc = extraKeys[key] = (cm) => {
      const fileTree = createFileTree(cm)
      console.log('run-script:',fileTree,cm,editor)
      // setTimeout(()=>{
      ...
      // },5000)
      if(fileTree) localStorage.setItem('jscad:script',fileTree[0].source)
      if (fileTree) editorCallbackToStream.callback({ type: 'read', id: 'loadRemote', data: fileTree })
    }
    window.g_get_script = function(){
    ...
    if(!window.g_init_script && localStorage.getItem('jscad:script')){
      window.g_init_script = true
      console.log('read source from jscad:script')
      let source = localStorage.getItem('jscad:script')
  ...
  if (state.design && state.design.filesAndFolders) {
    if (state.design.filesAndFolders.length === 1) {
      ...
      // if(localStorage.getItem('jscad:script')){
      //   console.log('read source from jscad:script')
      //   source = localStorage.getItem('jscad:script')
      //   window.g_get_script = function(){
      //     return source
      //   }
      //   setTimeout(() => {
      //     if (source) editorCallbackToStream.callback({ type: 'read', id: 'loadRemote', data: source })
      //   }, 300);
      // }
