...
class PopRuntime{
    ...
    */
    ...
    initFuncs(){
        ...
        //下载文件s
        this.binderAddOpcode('local.file.download',async function(context,opcode,token_x,token_y,opval,extra_data,...etc){ //token_x,token_y,opval,extra_data,...etc){
    ...
    async op(context,opcode,...etc){
        ...
        try{
            ...
            //一般而言，token_x指向的变量保存参数，token_y保存的是appid_key等信息，同时保存了urlbase等。
                ...
                for(let i=4;etc && i<etc.length;i++) param[etc[i]] = context[etc[i]]
                console.log('op-params:' + JSON.stringify(param) )
    ...
    {
        //console.log('opcode-isexists:'+opcode+' ret:'+(this.OPCODES[opcode]!=null))
