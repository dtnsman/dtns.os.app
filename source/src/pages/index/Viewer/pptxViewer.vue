<template>
  ...
  export default {
    ...
    beforeRouteLeave(to,from,next){
      console.log('into pptxViewer beforeRouteLeave')
      this.exit()
      next();
    },
    async created(){
    ...
    methods: {
      ...
      exit()
      {
        if(this.dxibManager)
        {
          ...
          this.dxib = null
          console.log('pptxViewer call dxibManager.exit()')
        }
      },
      copyFile()
      {
        if(this.dxibManager)
        {
          ...
          if(!localStorage.getItem('pptx-filename')) return this.$toast('无法复制文件，该文件URL为空！')
          window.g_folder_copy_data = {url:localStorage.getItem('pptx-filename')}
          this.$toast('复制成功，请切换到文件夹目录粘贴！')
        }
      },
      download(){
      ...
      async updateActionInfo()
      {
        ...
        if(window.g_now_action_info)
        {
          ...
          localStorage.setItem('pptx-filename',this.actionInfo.url)
          ...
          if(!cachedFileItem ) return this.$toast.fail('查看失败:PPTX文件不存在');
          this.title = cachedFileItem.data.filename
          this.src = cachedFileItem.data.filedata
        }
        this.url = 'static/js/pptjs-1.21.1/pptx.html'
      },
      rotationAdd()
      {
        this.rotation += 90
