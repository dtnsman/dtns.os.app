<template>
    ...
    <div v-if="showRTCChat" style="width: 100%;height:500px;overflow-y: auto;">
      <RTCChat></RTCChat>
    </div>
  ...
  export default {
    ...
    activated(){
      console.log('into x3dPlayer-activated:',window.g_now_action_info,window.g_now_start_3d_player)
      //window.g_now_start_3d_player = true
      //setTimeout(()=>window.g_now_start_3d_player=false,1000)
      if(window.g_now_action_info || window.g_now_start_3d_player){
        //避免使用物理按键回退的
        this.exit()

        //create 函数初始化
        // if(typeof window.g_x3dplayer_onPopCreatorMessage == 'function')
        // {
        //   window.removeEventListener('message',g_x3dplayer_onPopCreatorMessage)
        // }
        // window.g_x3dplayer_onPopCreatorMessage = this.onPopCreatorMessage
    ...
    async mounted(){
        console.log('x3dPlayer is mounted now!')
        ...
        let This  = this
        window.g_3d_show_rtcchat = function(chatid)
        {
          console.log('into g_3d_show_rtcchat:',chatid)
          // this.$refs.xdocument.style.height = '200px'
          localStorage.setItem('chatid',chatid)
          This.showRTCChat = true
        }
        window.g_3d_hide_rtcchat = function()
        {
          console.log('into g_3d_hide_rtcchat')
          This.showRTCChat = false
        }
    },
    beforeRouteLeave(to,from,next){
      console.log('into beforeRouteLeave,from&to:',from,to)
      //2024-4-28-keepalive
    ...
    methods: {
        ...
        exit()
        {
          ...
          try{
          if(this.player_dom)
            this.$refs.xdocument.removeChild(this.player_dom)
            this.$refs.xdocument.innerHTML = ''
          }catch(ex){console.log('this.$refs.xdocument.removeChild-ex:'+ex,ex)}
          this.xverse_json = null

          console.log('into exit')
          ...
          if(this.dxibManager)
          {
            ...
            this.dxib = null
            console.log('x3dPlayerViewer call dxibManager.exit()')
          }
        },
        ...
        editFile()
        {
          if(typeof window.g_player_stop_func=='function')
          {
            console.log('x3dplayer.vue-call-editFile,and call window.g_player_stop_func()!')
            window.g_player_stop_func()
          }
          ifileDb.deleteDataByKey('from.dtns.3d.creator.json')
          ifileDb.addData({key:'from.dtns.3d.creator.json',data:this.xverse_json})
        ...
        download(){
          ...
          else
          {
            ...
            rpc_client.downloadFileByBinary('x3dPlayer-'+Date.now()+'.xverse.json',uint8Array)
          }
        },
        setPlayer()
        {
        ...
        async updateActionInfo()
        {
          ...
          if(window.g_now_action_info)
          {
            ...
            try{
              ...
              console.log('3dPlayer.vue-loadActionResFile-fileData:',this.fileData,text,this.xitem)
            }catch(ex)
            {
