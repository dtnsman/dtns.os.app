<template>
    ...
    <div class="app-content" style="position: fixed;top:53px;bottom: 0px;left: 0;right: 0">
        <iframe ref="fabricIF" :src="url" style="width: 100%;height: 100%;" allow='microphone;camera;midi;encrypted-media;'></iframe>
    </div>
    </div>
  </template>
  <script>
  ...
  export default {
    ...
    beforeRouteLeave(to,from,next){
      console.log('into fabricViewer beforeRouteLeave')
      this.exit()
      next();
    },
    async created(){
      if(typeof window.g_fabric_onPopCreatorMessage == 'function')
      {
        window.removeEventListener('message',g_fabric_onPopCreatorMessage)
      }
      window.g_fabric_onPopCreatorMessage = this.onPopCreatorMessage
      window.addEventListener('message',this.onPopCreatorMessage, false);

      this.updateActionInfo()
    },
    async mounted(){
        console.log('fabricViewer is mounted now!')
    },
    beforeRouteLeave(to,from,next){
    ...
    methods: {
      ...
      exit()
      {
        if(this.dxibManager)
        {
          ...
          this.dxib = null
          console.log('fabricViewer call dxibManager.exit()')
        }
      },
      copyFile()
      {
        if(this.dxibManager)
        {
          ...
          if(!localStorage.getItem('fabric-url')) return this.$toast('无法复制文件，该文件URL为空！')
          window.g_folder_copy_data = {url:localStorage.getItem('fabric-url')}
          this.$toast('复制成功，请切换到文件夹目录粘贴！')
        }
      },
      async download(){
        if(this.dxibManager)
        {
          ...
          let fileItem = await window.ifileDb.getDataByKey('from.dtns.fabric.creator.json');
          if(!fileItem || !fileItem.data || !fileItem.data.json)
          {
            this.$toast('无法获得fabric源文件内容！')
            return 
          }
          let encoder = new TextEncoder()
          let uint8Array = encoder.encode(JSON.stringify(fileItem.data.json))
          rpc_client.downloadFileByBinary('fabric-'+Date.now()+'.fabric.json',uint8Array)
        }
      },
      async updateActionInfo()
      {
        ...
        if(window.g_now_action_info)
        {
          ...
          this.dxibName = !this.dxibManager.getDXIBObj() ?  '查看fabric画报文档': this.dxibManager.getDXIBObj().xmsg ? this.dxibManager.getDXIBObj().xmsg : this.dxibManager.getDXIBObj().name
          ...
          console.log('fabricViewer-fabric.json-text:',text,json,typeof filesDb)
          if(typeof filesDb!='undefined') console.log('filesDb.db:',filesDb.db)

          ifileDb.deleteDataByKey('from.dtns.fabric.creator.json')
          ifileDb.addData({key:'from.dtns.fabric.creator.json',data:{img:null,json:json}})

          localStorage.setItem('fabric-url',this.actionInfo.url)
        }
        else{
          this.title = 'fabric手绘文档编辑器'
        }
        this.isLoading = false
        this.url = 'static/js/creator.fabric/dist/index.html'
      },
    //   startRecordEditor()
    //   {
    ...
    //     this.recording = true
    //     this.$refs.fabricIF.contentWindow.postMessage(JSON.stringify({type:'g_fabric_start_record_editor',params:null}),'*')
    //   },
    //   startRecord(){
    //     console.log('startRecord:',this.recording)
    //     if(this.recording) return 
    //     this.recording = true
    //     this.$refs.fabricIF.contentWindow.postMessage(JSON.stringify({type:'g_fabric_start_record',params:null}),'*')
    //   },
    //   async endRecord(){
    //     console.log('endRecord:',this.recording)
    //     if(!this.recording) return
    //     this.recording = false
    //     this.$refs.fabricIF.contentWindow.postMessage(JSON.stringify({type:'g_fabric_stop_record',params:null}),'*')
    //   },
      async onPopCreatorMessage(e){//目标，保存来自pop-creator的数据，并保存为文件，然后进入minicard-poster页面进行上传
        console.log('fabricViewer.vue recv-message:',e.data,e)
        try{
      ...
      async processMessage(e)
      {
        ...
        try{
          ...
          console.log('fabricViewer.vue-onPopCreatorMessage-exception:'+ex,ex)
        }
        //进入的是3dEditor的处理浏览
        if(msgObj && msgObj.xtype=='dtns.fabric.creator')
        {
            ...
            let jsonItem = await ifileDb.getDataByKey('from.dtns.fabric.creator.json')
            ...
            //上传，推送至头榜
            let filename = '在线fabric文档'+Date.now()+'.fabric.json'
            const encoder = new TextEncoder();
            let u8arr = encoder.encode(JSON.stringify(jsonItem.data.json))
            if(!confirm('确定上传fabric文件吗？大小为：'+(Math.round(u8arr.byteLength*1.0/1024/1024,2)+'MB')))
            {
              return ;
            }
            ...
            console.log('fabric-json-File:',data.file)
            if(this.is_fabric_locked) return console.log('this.is_xdoc_locked is true')
            this.is_fabric_locked = true
            ...
            console.log('send-fabric-file-res:',res)
            this.is_fabric_locked = false
            if(!res || !res.ret){
                return this.$toast.fail('上传fabric文档失败' +(res ?res.msg:'未知网络错误'),3000)
            }
            ...
            let img_url = null
            let img_filename = 'fabric-'+Date.now()+'.fabric.webp'
            if(jsonItem && jsonItem.data && jsonItem.data.img && jsonItem.data.img.length>100)
            {
                let base64 = jsonItem.data.img 
                ...
                console.log('fabricViewer-webp-File:',file)
                let res = await new Promise((resolve)=>{
                rpc_client.sendImage(fileInfo,function(udata){
                    console.log('fabricViewer-sendImg-callback-data:'+JSON.stringify(udata))
                    if(udata && udata.data) resolve(udata.data)
                    else resolve(null)
                })
                })
                console.log('fabricViewer-webp-send-res:',res)
