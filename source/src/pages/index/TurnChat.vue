...
export default {
  ...
  methods: {
    ...
    async onClickRight(){//转让群主
      ...
      let res =  await this.$api.network.ChatManagerChange(user)
      if(res.ret){
        this.$toast.success('转让成功')
        this.$router.go(-1)
      }else if(res.msg == "change error : your is already owner of chatroom "){
        this.$toast.fail('转让失败，无法转让给自己')
      }else{
        this.$toast.fail('转让失败' +res.msg)
      }
    //   console.log(res)
    },
    ////
